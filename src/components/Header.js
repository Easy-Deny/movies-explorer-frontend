    ...
    return (
        <header className="header" style={{ background: (props.greenTheme ? '#073042' : 'none') }}>
            ...
            <div className="header__account">
                <Link to="/sign-up" className={`header__button ${!props.loggedIn ?'header__button_hide': ''}`}>Регистрация</Link>
                <Link to="/sign-in" className={`header__button header__button_green ${!props.loggedIn ?'header__button_hide': ''}`}>Войти</Link>
                <Link to="/profile" className={`header__button header__account-button ${props.loggedIn ?'header__button_hide': ''}`}>
